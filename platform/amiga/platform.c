...
// Interrupts originating from each CIA are multiplexed/nested within CPU & chipset-level IRQs.
// These values correspond to Paula's interrupt bits.
enum {
...
static inline bool is_cia_a_irq(unsigned irq) {
   return (irq >= (NUM_IRQS_PAULA + 1) && irq <= (NUM_IRQS_PAULA + NUM_IRQS_CIA));
}
...
enum handler_return m68k_platform_irq(uint8_t m68k_irq) {
   ...
   uint32_t combined = (uint32_t)(paula_this_level & ~((1 << CIA_A_MUX_LEVEL) | (1 << CIA_B_MUX_LEVEL)));

   uint16_t to_clear = 0;

   /* 
    * Read from CIA ICRs only when corresponding Paula interrupts have fired (IRQ 4 & 14).
    * This read also causes CIA interrupt state and ICR to be cleared.
    *
